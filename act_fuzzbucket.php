...
function run( $wordlist, $term, $glue, $mode )
{
	...
	$cmd = 'wfuzz -t 5 -o gcustom -z file,'.$wordlist.' --hc 400,404,500,503 https://s3.amazonaws.com/'.$t;
	echo $cmd."\n\n";
	...
	$m = preg_match_all( '#[0-9]+:[\s]+C=([0-9]+)[\s]+([0-9]+) L[\s]+([0-9]+) W[\s]+([0-9]+) Ch[\s]+"(.*)"#i', $output, $match );
	//var_dump( $match );

	if( $m ) {
		echo "\n";
		foreach( $match[5] as $m ) {
