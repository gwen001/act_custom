...
function clean() {
	global $html_path, $screenshot_path;
...
{ // check host/server
	...
	$screenshot_name  = urlencode($url).'.'.SCREENSHOT_EXTENSION;
	$html_name  = urlencode($url).'.'.HTML_EXTENSION;
...
{ // call httpscreenshot and parse output
	...
	{
		if( resizeAndCopy($screenshot_path,$destination_path) )
		{
